...
export default function EmployeesPage() {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('employees').insert([{ ...formData, image_url: imageUrl }])
      if (!error) {
        toast.success('Employee added successfully! 🎉', { id: loadingToast })
        setShowAddModal(false)
        setFormData({ name: '', role: '', department: '', email: '', status: 'Active', image_url: '' })
        setImageFile(null)
        setImagePreview('')
        fetchEmployees()
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from('employees').update({ ...formData, image_url: imageUrl }).eq('id', editingEmployee.id)
      if (!error) {
  ...
  const openEditModal = (emp: Employee) => {
    ...
    setFormData({ name: emp.name, role: emp.role, department: emp.department, email: emp.email, status: emp.status, image_url: emp.image_url || '' })
            ...
            {employees.map(emp => (
              <div key={emp.id} className="bg-white rounded-lg border border-slate-200 p-4 hover:shadow-md transition-shadow">
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <span className={`text-xs font-semibold px-2 py-1 rounded-full whitespace-nowrap ${emp.status === "Active" ? "bg-green-100 text-green-700" : emp.status === "On Leave" ? "bg-orange-100 text-orange-700" : "bg-red-100 text-red-700"}`}>
                    {emp.status}
                  </span>
                </div>
                {isAdmin && (
                ...
                <tbody className="divide-y divide-slate-200">
                  {employees.map(emp => (
                    <tr key={emp.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 xl:px-6 py-4">
                        <span className={`text-xs font-semibold px-3 py-1 rounded-full ${emp.status === "Active" ? "bg-green-100 text-green-700" : emp.status === "On Leave" ? "bg-orange-100 text-orange-700" : "bg-red-100 text-red-700"}`}>
                          {emp.status}
                        </span>
                      </td>
                      {isAdmin && (
        ...
        <form onSubmit={handleEdit} className="space-y-4">
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button
              ...
              className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium transition-colors disabled:opacity-50"
            >
              {uploadingImage ? 'Uploading...' : 'Update Employee'}
            </button>
          </div>
        </form>
      </div>
